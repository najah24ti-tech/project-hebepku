...
export default function InputPenjualan() {
  ...
  const handleChange = (e) => {
    ...
    if (name === "jumlah" || name === "harga") {
      const jumlah = Number(name === "jumlah" ? value : newData.jumlah);

      const harga = Number(name === "harga" ? value : newData.harga);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/penjualan", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Input Penjualan</h1>
        ...
        <div className="grid grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input
              ...
              readOnly
              value={(
                Number(formData.jumlah || 0) * Number(formData.harga || 0)
